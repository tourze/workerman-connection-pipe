...
$receiver->onWorkerStart = function ($worker) use (&$port): void {
    // 获取系统分配的端口
    ...
    // 当连接建立时发送ping
    $udp_connection->onConnect = function ($connection) use ($port): void {
        echo "UDP连接已建立\n";
        // 在ping消息中包含监听端口
    ...
    $udp_connection->onError = function ($connection, $code, $msg): void {
        echo "连接错误: [{$code}] {$msg}\n";
    };
...
// 当收到数据时
$receiver->onMessage = function ($connection, $data): void {
    echo '收到原始数据: ' . $data . "\n";
