...
// 创建日志处理器
$logger = new \Monolog\Logger('udp_to_udp');
...
$udpServer->onWorkerStart = function ($worker) use (&$connections, $targetAddress, $enableFullconeNat, $natTimeout) {
    // 在启动时清空连接数组
    $connections = [];
    ...
    $targetConnection->onConnect = function ($targetConn) use ($worker, &$connections) {
        echo "已连接到目标UDP服务器\n";
        // 存储共享的目标连接
        $worker->targetConnection = $targetConn;
    };

    $targetConnection->onMessage = function ($targetConn, $data, $targetAddress, $targetPort) use ($worker, &$connections) {
        echo "从目标UDP服务器收到回复: " . substr($data, 0, 20) . (strlen($data) > 20 ? "..." : "") . PHP_EOL;

        // 查找对应的源客户端（如果使用FullconeNAT，管道会自动处理回复路由）
    };

    $targetConnection->onClose = function () use ($worker) {
        echo "目标UDP服务器连接已关闭\n";
        // 尝试重新连接
    ...
    $targetConnection->onError = function ($targetConn, $code, $msg) {
...
$udpServer->onMessage = function ($connection, $data, $clientAddress, $clientPort) use (&$connections, $targetAddress, $enableFullconeNat, $natTimeout) {
...
$targetServer->onMessage = function ($connection, $data, $clientAddress, $clientPort) {
