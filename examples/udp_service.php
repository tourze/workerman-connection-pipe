...
$worker->onMessage = function (UdpConnection $connection, $data) {
    ...
    if (strpos($data, 'ping:') === 0) {
        ...
        // 记录客户端地址信息
        list($clientAddress) = explode(':', $connection->getRemoteAddress());
        ...
        // 每秒发送一次pong，总共发送10次
        $timer_id = Timer::add(1, function () use ($clientAddress, $clientPort, &$count, &$timer_id) {
            $count++;

            // 创建一个异步UDP连接发送pong
            $pong_connection = new AsyncUdpConnection("udp://{$clientAddress}:{$clientPort}");
            $pong_connection->onConnect = function ($connection) use ($count, $clientPort) {
                echo "发送第 {$count} 次pong到端口 {$clientPort}\n";
                $connection->send('pong');
                Timer::add(0.1, function () use ($connection) {
                    $connection->close();
                }, [], false);
            };
            ...
            $timestamp_connection->onConnect = function ($connection) use ($count, $clientPort) {
